...
print(network_image_height_depth, network_image_width_depth)
...
print(network_image_height_detection, network_image_width_detection)
...
while cap.isOpened():
    ...
    if not paused:
        ...
        if(bounding_box.miss_cnt > 10):
            ...
            # 차량 감지된 부분만을 대상으로 깊이 추정
            roi = frame[bounding_box.ymin:bounding_box.ymax, bounding_box.xmin:bounding_box.xmax]
            if roi.size == 0:
                continue  # Skip empty ROI

            # 중심점 좌표를 이용하여 깊이(거리) 추정
            resized_image_depth = cv2.resize(src=roi, dsize=(network_image_width_depth, network_image_height_depth))
...
# 코드 실행이 완료되었습니다. 총 소요된 시간: 79.6027660369873 초