...
class _DashBoardMain(QWidget):
    ...
    def __init__(self, parent, size: tuple | list = (800, 600), hide_creator_button: bool = False,
...
class _DashBoardContolsDesign(QWidget):
    ...
    def header_properties(self):
        self.header_border_color_lst = (QColorConstants.Svg.orchid, QColorConstants.Svg.red, QColorConstants.Svg.green)
    ...
    def break_properties(self):
        self.break_state = 0
        self.break_color_lst = (QColor(67, 13, 13, 200), QGradient(QGradient.Preset.ColorfulPeach), QColorConstants.Svg.green)

    def set_break_state(self, val, header_border_color=None):
    ...
    def paintEvent(self, event):
        ...
        if self.left_sign_detected:
            painter.drawPixmap(500, 150, self.unprotected_left_image)
...
class ServerDashBoard(QMainWindow):
    def __init__(self):
        ...
        self.dash_board.setFixedSize(770,450)
        ...
        self.receive_data_flag = False  # Flag to control data reception
        ...
        self.pwm = GPIO.PWM(self.buzzer, 262)
        self.pwm.start(0)  # 초기에는 소리가 나지 않도록 설정

    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Left:
            self.dash_board.dash_board_widget.dash_board_design_widget.indicator_triger(0)  # Left indicator
            if self.receive_data_flag==True:
                self.receive_data_flag = False  # Set flag to True when Key_Left is pressed
            else:
            ...
            self.receive_data_flag = False  # Reset flag when Key_Right is pressed
    ...
    def receive_data(self):
        if not self.receive_data_flag:
            return  # Exit if flag is not set
    ...
    def handle_data(self, received_data):
        ...
        self.dash_board.dash_board_widget.dash_board_design_widget.set_break_state(0, 0)
        if left_sign_detected:        
            ...
            if current_green_light:
                ...
                self.dash_board.dash_board_widget.dash_board_design_widget.set_horn_state(0)
                if depth_value >= 50:
                    self.dash_board.dash_board_widget.dash_board_design_widget.set_break_state(1, 1)  # 브레이크 활성화
            ...
            #self.pwm.ChangeDutyCycle(50.0)  # 소리 켜기
                ...
                if left_sign_detected:
                    ...
                    self.pwm.start(50.0)  # 소리 끄기
                    self.pwm.ChangeFrequency(162)
                    time.sleep(0.1)
                    self.pwm.ChangeFrequency(262)
                    self.pwm.stop()
        else:
            self.dash_board.dash_board_widget.dash_board_design_widget.set_left_sign_detected(left_sign_detected)
    def __del__(self):
