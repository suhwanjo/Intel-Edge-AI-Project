...
class VideoProcessor:
    def __init__(self, video_path, model_depth_xml_path, model_detection_xml_path, model_traffic_xml_path, raspberry_pi_ip, port=5001):
        # Initialize the video processor with paths to the models, video, and Raspberry Pi details
    ...
    def frame_capture(self):
        # Capture frames from the video stream and put them in the frame queue
        ...
        self.frame_queue.put(None)  # Signal end of frame capture by putting None in the queue

    def send_data_to_raspberry_pi(self, data):
        # Send data to the Raspberry Pi over a socket connection
    ...
    def run(self):
        # Create and start threads for frame capture, traffic sign detection, and vehicle detection
        ...
        while True:
            try:
                ...
                if frame is None:  # Check for end signal
                    break
            except queue.Empty:
                continue
            ...
            if self.frame_count % self.process_interval == 0:
                # Crop the frame for traffic detection
                ...
                if left_sign_detected and current_green_light:
                    # If both left sign and green light are detected, measure vehicle distance
                    ...
                    # Send data if only the left sign or green light is detected
                    data = {
                        "left_sign_detected": left_sign_detected,
                        "current_green_light": current_green_light,
                        "depth_value": 0
